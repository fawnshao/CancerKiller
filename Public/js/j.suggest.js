	(function($) {
		...
		$.suggest = function(input, options) {
			...
			if($.trim($input.val())=='' || $.trim($input.val())=='例：ERBB2') $input.val('例：ERBB2').css('color','#aaa');
			...
			function displayItems(items) {
				...
				if (items=='') {//热门基因遍历
					...
					html='<div class="gray ac_result_tip">热门基因</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
					...
					for (var i = 0; i < options.source.length; i++) {//基因匹配
					...
					if (html == '') {
						...
						suggest_tip = '<div class="gray ac_result_tip">' + items + '，按字母排序</div>';
					}
